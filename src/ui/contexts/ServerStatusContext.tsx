...
export function ServerStatusProvider({
  ...
  const refreshStatuses = useCallback(async () => {
    ...
    try {
      ...
      // Keep the last known status. A transient proxy/status check failure
      // should not paint all working hosts as degraded/orange.
    } finally {
  ...
  const getStatus = useCallback(
    (hostId: number): StatusValue => {
      ...
      return statuses.get(hostId)?.status || "online";
    },
