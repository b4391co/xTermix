...
export function TopNavbar({
  ...
  const groupedTabs = React.useMemo(() => {
    ...
    for (const tab of tabs) {
      ...
      grouped.push(rootTab || tab);
      seenRoots.add(rootId);
    }
    return grouped;
  }, [tabs, getSessionRootForTab]);

  const groupedTabIndexes = React.useMemo(
    () => groupedTabs.map((tab) => tabs.findIndex((t) => t.id === tab.id)),
    [groupedTabs, tabs],
  );
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    const fromDisplayIndex = dragState.draggedIndex;
    const toDisplayIndex = dragState.targetIndex;
    const fromIndex =
      fromDisplayIndex !== null ? groupedTabIndexes[fromDisplayIndex] : null;
    const toIndex =
      toDisplayIndex !== null ? groupedTabIndexes[toDisplayIndex] : null;
  ...
  return (
    <div>
      ...
      >
        ...
        >
          {groupedTabs.map((tab: TabData, index: number) => {
            if (tab.type === "split_picker") return null;
            const isActive = tab.id === currentRootTabId;
            const isSplit =
              tab.id === currentRootTabId &&
        ...
        <div className="flex items-center justify-center gap-2 flex-1 px-2">
          ...
          <Button
            ...
            className="w-[30px] h-[30px] border-edge text-xs font-semibold"
            title="Split vertical (Alt + D)"
          >
            V
          </Button>
          <Button
